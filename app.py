...
def show_graphs(symbol, market):
    ...
    if os.path.exists(daily_path):
        ...
        fig_price.update_traces(name='Close', showlegend=True)
        fig_price.update_layout(height=400)
        fig_price.update_xaxes(showgrid=True, gridwidth=1, gridcolor='LightPink', title_text=None)
        fig_price.update_yaxes(showgrid=True, gridwidth=1, gridcolor='LightPink', title_text=None)
        ...
        fig_rsi.update_traces(name='RSI', showlegend=True)
        fig_rsi.update_layout(height=400)
        fig_rsi.update_xaxes(showgrid=True, gridwidth=1, gridcolor='LightPink', title_text=None)
        fig_rsi.update_yaxes(showgrid=True, gridwidth=1, gridcolor='LightPink', title_text=None)
...
column_config_kr = {
    ...
    "섹터트렌드": st.column_config.TextColumn(width="small"),
    ...
    "캔들(5일)": st.column_config.TextColumn(width=120),
    ...
    "섹터트렌드체크": st.column_config.TextColumn(width="small"),
}
...
with main_tabs[0]:  # 필터 탭
    ...
    with st.form(key="filter_form"):
        ...
        with col4:
            ...
            sector_trend_apply = st.checkbox("섹터트렌드(해외전용)")
        ...
        if submitted:
            ...
            df_filter = df_filter.rename(columns={k: v for k, v in {
                ...
                'sector_trend_check': '섹터트렌드체크'
            }.items() if k in df_filter.columns})
            ...
            if not df_kr_results.empty:
                cols_kr = ['종목코드', '회사명', '섹터', '섹터트렌드', '시장', 'RSI_3일_2ago', 'RSI_3일_1ago', 'RSI_3일_latest', '종가', '시가총액', '업데이트', '20일평균거래대금', '오늘거래대금', '회전율', '캔들(5일)', '외국인순매수_3일전', '외국인순매수_2일전', '외국인순매수_1일전', 'PER_TTM', 'EPS_TTM', 'OBV_상승', 'RSI_3상승', 'RSI_3하강', '거래대금_상승', 'EPS > 0', '3<=PER<=30', '외국인 순매수', '캔들', '섹터트렌드체크']
            ...
            st.session_state.filter_results_kr = df_kr_results
            
            if not df_us_results.empty:
                cols_us = ['종목코드', '회사명', '섹터', '섹터트렌드', '시장', 'RSI_3일_2ago', 'RSI_3일_1ago', 'RSI_3일_latest', '종가', '시가총액', '업데이트', '20일평균거래대금', '오늘거래대금', '회전율', '캔들(5일)', '외국인순매수_3일전', '외국인순매수_2일전', '외국인순매수_1일전', 'PER_TTM', 'EPS_TTM', 'OBV_상승', 'RSI_3상승', 'RSI_3하강', '거래대금_상승', 'EPS > 0', '3<=PER<=30', '외국인 순매수', '캔들', '섹터트렌드체크']
            ...
            st.session_state.filter_results_us = df_us_results
    ...
    if not st.session_state.filter_results_kr.empty:
        ...
        st.dataframe(pd.DataFrame(), column_config=column_config_kr)
    ...
    if not st.session_state.filter_results_us.empty:
        ...
        st.dataframe(pd.DataFrame(), column_config=column_config_us)
        ...
        if df_back.empty:
            ...
            def apply_rename_format_and_unit(df, market_type):
                ...
                if market_type == 'KR':
                    ...
                    cols = [
                        '종목코드', '회사명', '섹터', '섹터트렌드', '시장', 'RSI_3일_2ago', 'RSI_3일_1ago', 'RSI_3일_latest',
            ...
            with back_sub_tabs[0]:  # 장기
                ...
                if not kr_long_back.empty:
                    # ✅ 업데이트 날짜순으로 정렬 (최신순)
                    kr_long_back = kr_long_back.sort_values('업데이트', ascending=False)
                ...
                if not us_long_back.empty:
                    # ✅ 업데이트 날짜순으로 정렬 (최신순)
                    us_long_back = us_long_back.sort_values('업데이트', ascending=False)
            ...
            with back_sub_tabs[1]:  # 단기
                ...
                if not kr_short_back.empty:
                    # ✅ 업데이트 날짜순으로 정렬 (최신순)
                    kr_short_back = kr_short_back.sort_values('업데이트', ascending=False)
                ...
                if not us_short_back.empty:
                    # ✅ 업데이트 날짜순으로 정렬 (최신순)
                    us_short_back = us_short_back.sort_values('업데이트', ascending=False)
            ...
            with back_sub_tabs[2]:  # 중기
                ...
                if not kr_mid_back.empty:
                    # ✅ 업데이트 날짜순으로 정렬 (최신순)
                    kr_mid_back = kr_mid_back.sort_values('업데이트', ascending=False)
                ...
                if not us_mid_back.empty:
                    # ✅ 업데이트 날짜순으로 정렬 (최신순)
                    us_mid_back = us_mid_back.sort_values('업데이트', ascending=False)
...
with main_tabs[2]:  # KR 탭
    ...
    with kr_sub_tabs[0]:  # 장기
        ...
        if not df_long_full.empty:
            ...
            df_long['sector_trend_check'] = df_long['sector_trend'].apply(lambda x: '✅' if '+' in str(x) else '❌' if '-' in str(x) else 'N/A')
            df_long['캔들(5일)'] = df_long['upper_closes'].astype(str) + ' (상단) / ' + df_long['lower_closes'].astype(str) + ' (하단)'
            
            df_long = df_long.rename(columns={k: v for k, v in {'symbol': '종목코드', 'market': '시장', 'name': '회사명', 'sector': '섹터', 'sector_trend': '섹터트렌드',
                       'rsi_d_2ago': 'RSI_3일_2ago', 'rsi_d_1ago': 'RSI_3일_1ago', 'rsi_d_latest': 'RSI_3일_latest', 
                       'close': '종가',
                       'market_cap': '시가총액', 'avg_trading_value_20d': '20일평균거래대금', 'today_trading_value': '오늘거래대금', 'turnover': '회전율',
                       'foreign_net_buy_3ago': '외국인순매수_3일전', 'foreign_net_buy_2ago': '외국인순매수_2일전', 'foreign_net_buy_1ago': '외국인순매수_1일전',
                       'per': 'PER_TTM', 'eps': 'EPS_TTM', 'obv_bullish_cross': 'OBV_상승', 'rsi_3up': 'RSI_3상승', 'rsi_3down': 'RSI_3하강', 'trading_high': '거래대금_상승',
                       'eps_positive': 'EPS > 0', 'per_range': '3<=PER<=30', 'cap_status': '업데이트', 'foreign_positive': '외국인 순매수', 'candle_upper_3': '캔들',
                       'sector_trend_check': '섹터트렌드체크'}.items() if k in df_long.columns})
    ...
    with kr_sub_tabs[1]:  # 단기
        ...
        if not df_short_full.empty:
            ...
            df_short['sector_trend_check'] = df_short['sector_trend'].apply(lambda x: '✅' if '+' in str(x) else '❌' if '-' in str(x) else 'N/A')
            df_short['캔들(5일)'] = df_short['upper_closes'].astype(str) + ' (상단) / ' + df_short['lower_closes'].astype(str) + ' (하단)'
            
            df_short = df_short.rename(columns={k: v for k, v in {'symbol': '종목코드', 'market': '시장', 'name': '회사명', 'sector': '섹터', 'sector_trend': '섹터트렌드',
                       'rsi_d_2ago': 'RSI_3일_2ago', 'rsi_d_1ago': 'RSI_3일_1ago', 'rsi_d_latest': 'RSI_3일_latest', 
                       'close': '종가',
                       'market_cap': '시가총액', 'avg_trading_value_20d': '20일평균거래대금', 'today_trading_value': '오늘거래대금', 'turnover': '회전율',
                       'foreign_net_buy_3ago': '외국인순매수_3일전', 'foreign_net_buy_2ago': '외국인순매수_2일전', 'foreign_net_buy_1ago': '외국인순매수_1일전',
                       'per': 'PER_TTM', 'eps': 'EPS_TTM', 'obv_bullish_cross': 'OBV_상승', 'rsi_3up': 'RSI_3상승', 'rsi_3down': 'RSI_3하강', 'trading_high': '거래대금_상승',
                       'eps_positive': 'EPS > 0', 'per_range': '3<=PER<=30', 'cap_status': '업데이트', 'foreign_positive': '외국인 순매수', 'candle_upper_3': '캔들',
                       'sector_trend_check': '섹터트렌드체크'}.items() if k in df_short.columns})
    ...
    with kr_sub_tabs[2]:  # 중기
        ...
        if not df_mid_full.empty:
            ...
            df_mid['sector_trend_check'] = df_mid['sector_trend'].apply(lambda x: '✅' if '+' in str(x) else '❌' if '-' in str(x) else 'N/A')
            df_mid['캔들(5일)'] = df_mid['upper_closes'].astype(str) + ' (상단) / ' + df_mid['lower_closes'].astype(str) + ' (하단)'
            
            df_mid = df_mid.rename(columns={k: v for k, v in {'symbol': '종목코드', 'market': '시장', 'name': '회사명', 'sector': '섹터', 'sector_trend': '섹터트렌드',
                       'rsi_d_2ago': 'RSI_3일_2ago', 'rsi_d_1ago': 'RSI_3일_1ago', 'rsi_d_latest': 'RSI_3일_latest', 
                       'close': '종가',
                       'market_cap': '시가총액', 'avg_trading_value_20d': '20일평균거래대금', 'today_trading_value': '오늘거래대금', 'turnover': '회전율',
                       'foreign_net_buy_3ago': '외국인순매수_3일전', 'foreign_net_buy_2ago': '외국인순매수_2일전', 'foreign_net_buy_1ago': '외국인순매수_1일전',
                       'per': 'PER_TTM', 'eps': 'EPS_TTM', 'obv_bullish_cross': 'OBV_상승', 'rsi_3up': 'RSI_3상승', 'rsi_3down': 'RSI_3하강', 'trading_high': '거래대금_상승',
                       'eps_positive': 'EPS > 0', 'per_range': '3<=PER<=30', 'cap_status': '업데이트', 'foreign_positive': '외국인 순매수', 'candle_upper_3': '캔들',
                       'sector_trend_check': '섹터트렌드체크'}.items() if k in df_mid.columns})
    ...
    with kr_sub_tabs[3]:  # Total
        ...
        if not df_ind.empty:
            ...
            df_kr_ind['sector_trend_check'] = df_kr_ind['sector_trend'].apply(lambda x: '✅' if '+' in str(x) else '❌' if '-' in str(x) else 'N/A')
            df_kr_ind['캔들(5일)'] = df_kr_ind['upper_closes'].astype(str) + ' (상단) / ' + df_kr_ind['lower_closes'].astype(str) + ' (하단)'
            
            col_map_total = {'symbol': '종목코드', 'market': '시장', 'name': '회사명', 'sector': '섹터', 'sector_trend': '섹터트렌드',
                             'rsi_d_2ago': 'RSI_3일_2ago', 'rsi_d_1ago': 'RSI_3일_1ago', 'rsi_d_latest': 'RSI_3일_latest', 
                             'close': '종가',
            ...
            cols_kr_total = ['종목코드', '회사명', '섹터', '섹터트렌드', '시장', 'RSI_3일_2ago', 'RSI_3일_1ago', 'RSI_3일_latest', '종가', '시가총액', '업데이트', '20일평균거래대금', '오늘거래대금', '회전율', '캔들(5일)', '외국인순매수_3일전', '외국인순매수_2일전', '외국인순매수_1일전', 'PER_TTM', 'EPS_TTM', 'OBV_상승', 'RSI_3상승', 'RSI_3하강', '거래대금_상승', 'EPS > 0', '3<=PER<=30', '외국인 순매수', '캔들', '섹터트렌드체크']
...
with main_tabs[3]:  # US 탭
    ...
    with us_sub_tabs[1]:  # 단기
        ...
        if not df_short_full.empty:
            ...
            df_short['foreign_positive'] = ((df_short['foreign_net_buy_1ago'] > 0) & (df_short['foreign_net_buy_2ago'] > 0)).apply(lambda x: '✅' if x else '❌')
            df_short['candle_upper_3'] = (df_short['upper_closes'] >= 3).apply(lambda x: '✅' if x else '❌')
            df_short['sector_trend_check'] = df_short['sector_trend'].apply(lambda x: '✅' if x else '❌')
            df_short = df_short.rename(columns={k: v for k, v in {'symbol': '종목코드', 'market': '시장', 'name': '회사명', 'sector': '섹터', 'sector_trend': '섹터트렌드',
                   'rsi_d_2ago': 'RSI_3일_2ago', 'rsi_d_1ago': 'RSI_3일_1ago', 'rsi_d_latest': 'RSI_3일_latest', 
                   'close': '종가',
                   'market_cap': '시가총액', 'avg_trading_value_20d': '20일평균거래대금', 'today_trading_value': '오늘거래대금', 'turnover': '회전율',
...
with us_sub_tabs[3]:  # Total
    ...
    if not df_ind.empty:
        ...
        df_us_ind['sector_trend_check'] = df_us_ind['sector_trend'].apply(lambda x: '✅' if '+' in str(x) else '❌' if '-' in str(x) else 'N/A')
        df_us_ind['캔들(5일)'] = df_us_ind['upper_closes'].astype(str) + ' (상단) / ' + df_us_ind['lower_closes'].astype(str) + ' (하단)'
        ...
        cols_us_total = ['종목코드', '회사명', '섹터', '섹터트렌드', '시장', 'RSI_3일_2ago', 'RSI_3일_1ago', 'RSI_3일_latest', '종가', '시가총액', '업데이트', '20일평균거래대금', '오늘거래대금', '회전율', '캔들(5일)', '외국인순매수_3일전', '외국인순매수_2일전', '외국인순매수_1일전', 'PER_TTM', 'EPS_TTM', 'OBV_상승', 'RSI_3상승', 'RSI_3하강', '거래대금_상승', 'EPS > 0', '3<=PER<=30', '외국인 순매수', '캔들', '섹터트렌드체크']
...
with main_tabs[4]:  # 로그 탭
    ...
    with st.expander("📋 필터 조건 및 알고리즘 설명서", expanded=True):
        st.markdown("""
        ### 🎯 이 도구의 목적
